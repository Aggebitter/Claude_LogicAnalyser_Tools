...
All logic lives in shared/ and is imported directly (no dependency on logic2 backend).
"""
...
from protocol_fingerprints import (  # noqa: F401
...
# GPIO marker templates per platform (duplicated from logic2 analysis to stay independent)
...
def run_assertions(assertions: list[dict], capture_path: str, client) -> list[dict]:
    results = []
    for assertion in assertions:
        ...
        results.append({"id": assertion["id"], "description": assertion["description"], **result})
    return results


def _evaluate_assertion(assertion: dict, capture_path: str, client) -> dict:
    kind = assertion["type"]
...
def _assert_timing(params: dict, capture_path: str, client) -> dict:
    ch = params["channel"]
    ...
    key_map = {
        "frequency": "frequency_hz", "period": "period_us",
        "pulse_high": "pulse_width_high_us", "pulse_low": "pulse_width_low_us",
    ...
    return {
        ...
        "detail": f"CH{ch} {measurement}: expected={expected}, actual={actual:.4f}, diff={pct_diff:.1f}%",
    }


def _assert_logic(params: dict, capture_path: str, client) -> dict:
    ...
    first_a = next((t for t, ch, lvl in edges if ch == ch_a and lvl == 0), None)
    first_b = next((t for t, ch, lvl in edges if ch == ch_b and lvl == 1), None)

    if first_a is None:
        return {"result": "ERROR", "detail": f"No transition on CH{ch_a}"}
    if first_b is None:
        return {"result": "ERROR", "detail": f"No transition on CH{ch_b}"}
    ...
    if relationship == "a_before_b":
        ...
        if passed and max_delay_us is not None and delay_us > max_delay_us:
...
def _assert_protocol(params: dict, capture_path: str, client) -> dict:
    decoder = params.get("decoder", params.get("analyzer", ""))
    channel_map = params.get("channel_map", {})
    options = params.get("decoder_options", params.get("settings", {}))
    frames = client.run_decoder(capture_path, decoder, channel_map=channel_map, options=options)

    if not frames:
        return {"result": "FAIL", "detail": f"No frames decoded by '{decoder}'"}

    expect_data = params.get("expect_data")
    if expect_data:
        for frame in frames:
            if expect_data.lower() in str(frame.get("data", "")).lower():
                return {"result": "PASS", "detail": f"Found expected data in frame: {frame}"}
        return {"result": "FAIL", "detail": f"Expected data '{expect_data}' not found in {len(frames)} frames"}

    return {"result": "PASS", "detail": f"{len(frames)} frames decoded. First: {frames[0]}"}


def inject_gpio_markers(source_file: str, function_name: str, gpio_pin: int, platform: str) -> dict:
    ...
    modified = (
        original[:match.end()]
        + f"\n    {marker_set}  /* profiling: {function_name} entry */"
        + original[match.end():]
    )
    modified = re.sub(
        r"(\breturn\b\s*[^;]*;)",
        rf"{marker_clear}  /* profiling: {function_name} exit */\n    \1",
        modified,
    )
    src_path.write_text(modified)
    return {"file": source_file, "function": function_name, "gpio_pin": gpio_pin,
            "platform": platform, "status": "injected"}


def profile_from_markers(capture_path: str, marker_channel: int, client) -> dict:
    ...
    for t, lvl in ch_edges:
        if lvl == 1:
            ...
            durations.append((t - entry_time) * 1e6)
            entry_time = None

    if not durations:
        return {"error": f"No complete toggle pairs on CH{marker_channel}"}
